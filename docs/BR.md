...
4. The CA is made aware of a demonstrated or proven method that can easily compute the Subscriber's Private Key (such as those identified in section 6.1.1.3(4));
...
5. The Public Key corresponds to an industry-demonstrated weak Private Key, in particular:
    ...
    2. In the case of Debian weak keys (https://wiki.debian.org/SSLkeys), the CA SHALL reject at least keys generated by the flawed OpenSSL version with all combinations of the following parameters:
        ...
        3. All RSA Public Key lengths supported by the CA up to and including 4096 bits and all ECDSA Public Key lengths supported by the CA up to and including 521 bits; and
