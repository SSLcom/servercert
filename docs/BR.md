...
4. The CA is made aware of a demonstrated or proven method that can easily compute the Subscriber's Private Key based on the Public Key (such as those identified in section 6.1.1.3(4));
...
2. There is clear evidence that the specific method used to generate the Private Key was flawed (see 6.1.1.3 (5) below);
...
5. The Public Key corresponds to an industry-demonstrated weak Private Key, in particular:
    ...
    b. In the case of Debian weak keys (https://wiki.debian.org/SSLkeys), the CA SHALL reject at least keys generated by the flawed OpenSSL version with all combinations of the following parameters:
...
Suggested tools that CAs MAY use to check for weak keys can be found here: https://cabforum.org/resources/tools/ 
